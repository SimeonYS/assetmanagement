...
class AassetmanagementPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `assetmanagement`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM assetmanagement WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO assetmanagement (date, title, link, content)"
